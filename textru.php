...
function my_activation() {
	...
	function do_this_hourly() {
		...
		foreach ( $featured->posts as $key => $val ) {
			if ( get_post_meta( $val->ID, "textru", true ) == '' ) {
				$postQuery                 = array();
				...
				if ( file_exists( ABSPATH . 'textru/' . $val->ID . '.txt' ) ) {
					...
					$current_text_file = '</br></br>' . '<b>ДАТА:</b> ' . $out_text->date_check . '</br>';
					$current_text_file .= '</br></br>' . '<b>Уникальность:</b> ' . $out_text->unique . '</br>';
					$current_text_file .= '</br></br>' . '<b>Чистый текст:</b> ' . $out_text->clear_text . '</br>';
					$current_text_file .= '</br></br>' . '<b>Микс слов:</b> ' . $out_text->mixed_words . '</br>';
					$current_text_file .= '</br></br>' . '<b>uid:</b> ' . get_post_meta( $val->ID, "textru", true ) . '</br>';
					if ( is_array( $out_text->urls ) ) {
						foreach ( $out_text->urls as $kkey => $vval ) {
